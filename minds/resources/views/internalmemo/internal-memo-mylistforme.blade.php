                                    ...
                                    <th scope="row">
                                     <div class="form-check font-size-16">
                                        ...
                                        @if ($datas->created_at->translatedFormat('m') == '01')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/I/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '02')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/II/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '03')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/III/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '04')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/IV/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '05')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/V/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '06')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/VI/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '07')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/VII/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '08')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/VIII/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '09')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/IX/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '10')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/X/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '11')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/XI/{{$datas->created_at->translatedFormat('y')}}</td>
                                        @elseif ($datas->created_at->translatedFormat('m') == '12')
                                            <td>{{$datas->nomor_internalmemo}}/CSV/{{$datas->user->divisi->name}}/XII/{{$datas->created_at->translatedFormat('s')}}</td>
                                        @endif
